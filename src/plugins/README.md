...
- **Per-plugin tests**: Add a test file with a unique name under your plugin, e.g. `src/plugins/<plugin_name>/test_<plugin_name>.py` (e.g. `test_browser.py`). Using a unique name avoids pytest collection clashes with other plugins. Use the shared fixtures from `src/plugins/conftest.py`:
...
3. Optionally add `src/plugins/<name>/test_<name>.py` for plugin-specific tests (e.g. test_browser.py). Use a unique filename so pytest does not confuse modules across plugins.
